...
figure(2);
plot(ORI(:,1),ORI(:,2),'ro');hold on;
plot(po(:,1),po(:,2),'bs');hold on;
this_c='k';
dis_flag=0;
...
if xu(1)<-100 %劣质解 %超载
        ...
        if length(kehu_idx)>0 %有客户列表
            ...
            for j=1:kehu_num %遍历每个客户
                if j==1%第一个客户
                    ...
                    plot([qian_po(1),this_po(1)],[qian_po(2),this_po(2)],[this_c,'-']);hold on;
            ...
            for k=1:length(road)
                word_r=[word_r,num2str(road(k)),'-'];
            end
            word_r=[word_r,'0'];
            
            disp(['第',num2str(ii),'辆车所属仓库为：',num2str(home_idx),'，配送路径为',word_r,'，总送货需求为',num2str(sum(need(kehu_idx))),'，总运送距离为：',num2str(all_d)]);
    ...
    f(1)=all_t*omg1+yunfei*omg2+dis_flag*1e10*yunfeixishu;
